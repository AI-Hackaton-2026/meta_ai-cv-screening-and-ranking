...
const COMPONENTS = {
  ...
  ol: ({ node, ...props }) => (
    <ol className="mb-3 last:mb-0 pl-5 list-decimal marker:text-(--muted-foreground) space-y-1.5" {...props} />
  ),
  li: ({ node, ...props }) => (
  ...
  blockquote: ({ node, ...props }) => (
    <blockquote className="mb-3 border-l-2 border-(--primary) pl-3 italic text-(--muted-foreground)" {...props} />
  ),
  code: ({ node, ...props }) => (
    <code className="rounded bg-(--primary-light) px-1.5 py-0.5 text-[0.85em] text-(--foreground)" {...props} />
  ),
};
